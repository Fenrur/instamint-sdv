...
export function useGetConversations() {
  const getConversationsFetcher = () => getConversations()
  const {data, error, mutate} = useSWR("getConversations", getConversationsFetcher)

  return {
    conversations: data,
    conversationsMutate: mutate,
    errorConversations: error,
  }
}

export function useGetConversationMessages(username: string, page: number = 0) {
  const getConversationMessagesFetcher = () => getConversationMessages(username, page)
  const {data, error, mutate} = useSWR(`getConversationMessages-${username}-${page}`, getConversationMessagesFetcher)
...
export function useSendPrivateMessage() {
  const sendPrivateMessageFetcher = (_: any, {arg}: { arg: SendPrivateMessageRequest }) => sendPrivateMessage(arg)
  const {trigger, data, error, isMutating} = useSWRMutation("sendPrivateMessage", sendPrivateMessageFetcher)

  return {
    sendMessage: (req: SendPrivateMessageRequest) => trigger(req),
    dataSendMessage: data,
    errorSendMessage: error,
    isSendingMessage: isMutating
  }
}
