...
export async function getConversations(): Promise<GetConversationsResponse | string> {
  const res = await fetch("/api/private-message", {
    method: "GET"
  })

  if (res.status === StatusCodes.OK) {
    return GetConversationsResponse.parse(await res.json())
  }
...
export async function getConversationMessages(username: string, page: number = 0): Promise<GetConversationMessagesResponse | string> {
  const url = encodeURI(`/api/private-message/${username}?page=${page}`)
  const res = await fetch(url, {
    method: "GET"
  })

  if (res.status === StatusCodes.OK) {
    return GetConversationMessagesResponse.parse(await res.json())
  }
  ...
  switch (errorCode) {
    ...
    case ErrorCode.CANT_MESSAGE_YOURSELF:
      return "cant_message_yourself"

    case ErrorCode.PRIVATE_PROFILE_NOT_FOLLOWING:
...
export async function sendPrivateMessage(req: SendPrivateMessageRequest): Promise<SendPrivateMessageResponse | string> {
  const res = await fetch("/api/private-message", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
  ...
  if (res.status === StatusCodes.CREATED) {
    return SendPrivateMessageResponse.parse(await res.json())
  }
  ...
  switch (errorCode) {
    ...
    case ErrorCode.REPLY_MESSAGE_NOT_FOUND:
      return "reply_message_not_found"

    case ErrorCode.INVALID_REPLY_MESSAGE:
